...
bool first = true;
vector<double> simple_it(const vector<vector<double>>& a, const vector<double>& b, double eps, int& it) {
    ...
    it = 0;
    vector<double> ans(a.size()), ans_prev;
    do {
        ans_prev = ans;
        ans = c * ans_prev + d;
        it++;
    } while (norm(ans + (-1 * ans_prev)) >= eps);
...
int main() {
    ...
    int it;
    vector<double> eps{ 1e-2, 1e-4, 1e-8, 1e-12 };
    for (double e : eps) {
        x = simple_it(a, b, e, it);
        cout << fixed << setprecision(5);
        cout << "Answer: (";
        for (double i : x)
            cout << i << ' ';
