...
double sum_xy_pown(const vector<pair<double, double>>& pts, int n) {
	double ans = 0;
	for (auto i : pts)
		ans += pow(log(i.first), n) * log(i.second);
	return ans;
}
...
int main() {
	...
	cout << "f:\n";
	for (double x = 1; x <= 3; x += 2.0 / 20)
		cout << f(x) << endl;
	cout << "-----------------------------------------\n";
	cout << "sq_min:\n";
	for (double x = 1; x <= 3.05; x += 2.0 / 20)
		cout << exp(a[0]) * pow(x, a[1]) << endl;
