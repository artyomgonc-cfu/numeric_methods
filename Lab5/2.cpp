...
int main() {
    ...
    matrix[0][2] = sum_xi_pown(pts, 2);

    matrix[1][0] = sum_xi_pown(pts, 1);
    matrix[1][1] = sum_xi_pown(pts, 2);
    matrix[1][2] = sum_xi_pown(pts, 3);

	matrix[2][0] = sum_xi_pown(pts, 2);
	matrix[2][1] = sum_xi_pown(pts, 3);
	matrix[2][2] = sum_xi_pown(pts, 4);

	matrix[0][3] = sum_xy_pown(pts, 0);
	matrix[1][3] = sum_xy_pown(pts, 1);
	matrix[2][3] = sum_xy_pown(pts, 2);
